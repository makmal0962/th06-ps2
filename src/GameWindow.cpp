...
extern "C" { 
    void pglBeginGeometry(); 
    void pglEndGeometry(); 
    void pglRenderGeometry(); 
    void init_scr();
...
#ifdef __PS2__
            printf("pglEndGeometry...\n");
            pglEndGeometry();
            printf("pglEndGeometry done\n");
            pglRenderGeometry();
            printf("pglRenderGeometry done\n");
#endif
...
{
// note for developers:
// we need to just render graphics on screen framebuffer.
// touhou 06 resolution is 640x480, but ps2 ntsc resolution available is 640x448. 
// just crop 16 pixels from top and bottom to make it 640x448, but first we need to move internal fps counter (located at bottom right corner).

#ifdef __PS2__
    // SDL_Init(SDL_INIT_JOYSTICK | SDL_INIT_GAMECONTROLLER);
    // SDL_Init(SDL_INIT_VIDEO);
    SDL_Init(0); // no video init, ps2gl will handle display
    // g_glFuncTable.ResolveFunctions(false); // populate ps2gl function pointers
    // g_GameWindow.window = (SDL_Window*)1; // dummy non-null
...
#ifdef __PS2__
    // TODO: force fulscreen on PS2
    // ignore windowed config, just always fullscreen.
...
i32 GameWindow::InitD3dRendering(void)
{
    ...
    //        }
    //    }
    printf("InitD3dDevice...\n");
    InitD3dDevice();
    printf("InitD3dDevice done\n");
